...
public class AliPayEnterpriseBillDaoImpl implements AliPayEnterpriseBillDao {
	...
	public void add(AliPayEnterpriseBill aliPayEnterpriseBill) {
		String sql = "insert into " + table + "( outBizNo, payType, account, amount, payerName, peyeeName, remark, orderId, payDate, createTime, createUser, status, reason, orderFee) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
		jdbc.updateForBoolean(sql, aliPayEnterpriseBill.getOutBizNo(),aliPayEnterpriseBill.getPayType(),aliPayEnterpriseBill.getAccount(),aliPayEnterpriseBill.getAmount(),aliPayEnterpriseBill.getPayerName(),aliPayEnterpriseBill.getPeyeeName(),aliPayEnterpriseBill.getRemark(),aliPayEnterpriseBill.getOrderId(),aliPayEnterpriseBill.getPayDate(),aliPayEnterpriseBill.getCreateTime(),aliPayEnterpriseBill.getCreateUser(),aliPayEnterpriseBill.getStatus(),aliPayEnterpriseBill.getReason(),aliPayEnterpriseBill.getOrderFee());
	}

	@Override
	public void update(AliPayEnterpriseBill aliPayEnterpriseBill) {
		String sql = "update " + table + " set  outBizNo = ? , payType = ? , account = ? , amount = ? , payerName = ? , peyeeName = ? , remark = ? , orderId = ? , payDate = ? , createTime = ? , createUser = ? , status = ? , reason = ? , orderFee = ?  where id = ?  ";
		jdbc.updateForBoolean(sql,aliPayEnterpriseBill.getOutBizNo(),aliPayEnterpriseBill.getPayType(),aliPayEnterpriseBill.getAccount(),aliPayEnterpriseBill.getAmount(),aliPayEnterpriseBill.getPayerName(),aliPayEnterpriseBill.getPeyeeName(),aliPayEnterpriseBill.getRemark(),aliPayEnterpriseBill.getOrderId(),aliPayEnterpriseBill.getPayDate(),aliPayEnterpriseBill.getCreateTime(),aliPayEnterpriseBill.getCreateUser(),aliPayEnterpriseBill.getStatus(),aliPayEnterpriseBill.getReason(),aliPayEnterpriseBill.getOrderFee(), aliPayEnterpriseBill.getId());
	}
	...
	public List<AliPayEnterpriseBill> page(Pager page, AliPayEnterpriseBillSearchForm aliPayEnterpriseBillSearchForm) {
		...
		return jdbc.queryForPage(buildCondition(sql, aliPayEnterpriseBillSearchForm, param), AliPayEnterpriseBill.class, page, param);
	}
	@Override
	...
	private String buildCondition(String sql, AliPayEnterpriseBillSearchForm aliPayEnterpriseBillSearchForm, StatementParameter param) {
		sql += " where 1 = 1";
        if (aliPayEnterpriseBillSearchForm.getStatus() != null 
        		&& StringUtils.isNotEmpty(aliPayEnterpriseBillSearchForm.getStatus()+"")) {
			sql += " and status  = ?";
			param.set(aliPayEnterpriseBillSearchForm.getStatus());
		}
        if (aliPayEnterpriseBillSearchForm.getOutBizNo() != null 
        		&& StringUtils.isNotEmpty(aliPayEnterpriseBillSearchForm.getOutBizNo()+"")) {
			sql += " and outBizNo  = ?";
			param.set(aliPayEnterpriseBillSearchForm.getOutBizNo());
		}
        if (aliPayEnterpriseBillSearchForm.getPayType() != null 
        		&& StringUtils.isNotEmpty(aliPayEnterpriseBillSearchForm.getPayType()+"")) {
			sql += " and payType  = ?";
			param.set(aliPayEnterpriseBillSearchForm.getPayType());
		}
        if (aliPayEnterpriseBillSearchForm.getAccount() != null 
        		&& StringUtils.isNotEmpty(aliPayEnterpriseBillSearchForm.getAccount()+"")) {
			sql += " and account  = ?";
			param.set(aliPayEnterpriseBillSearchForm.getAccount());
		}
        if (aliPayEnterpriseBillSearchForm.getPeyeeName() != null 
        		&& StringUtils.isNotEmpty(aliPayEnterpriseBillSearchForm.getPeyeeName()+"")) {
			sql += " and peyeeName  = ?";
			param.set(aliPayEnterpriseBillSearchForm.getPeyeeName());
		}
        if (aliPayEnterpriseBillSearchForm.getOrderId() != null 
        		&& StringUtils.isNotEmpty(aliPayEnterpriseBillSearchForm.getOrderId()+"")) {
			sql += " and orderId  = ?";
			param.set(aliPayEnterpriseBillSearchForm.getOrderId());
		}
        if (aliPayEnterpriseBillSearchForm.getCreateUser() != null 
        		&& StringUtils.isNotEmpty(aliPayEnterpriseBillSearchForm.getCreateUser()+"")) {
			sql += " and createUser  = ?";
			param.set(aliPayEnterpriseBillSearchForm.getCreateUser());
		}
        if (aliPayEnterpriseBillSearchForm.getStartCreateTime() != null 
        		&& StringUtils.isNotEmpty(aliPayEnterpriseBillSearchForm.getStartCreateTime()+"")) {
			sql += " and createTime  >= ?";
			param.set(aliPayEnterpriseBillSearchForm.getStartCreateTime());
		}
        if (aliPayEnterpriseBillSearchForm.getEndCreateTime() != null 
        		&& StringUtils.isNotEmpty(aliPayEnterpriseBillSearchForm.getEndCreateTime()+"")) {
			sql += " and createTime  <= ?";
