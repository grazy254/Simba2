...
public class AreaParam {
	...
	/**
	 * 子级行政区 规则：设置显示下级行政区级数（行政区级别包括：国家、省/直辖市、市、区/县、乡镇/街道多级数据） 可选值：0、1、2、3等数字，并以此类推
	...
	/**
	 * 返回结果控制 此项控制行政区信息中返回行政区边界坐标点； 可选值：base、all; base:不返回行政区边界坐标点；
	 * all:只返回当前查询district的边界值，不返回子节点的边界值； 目前不能返回乡镇/街道级别的边界值
	 */
	private String extensions;

	/**
	 * 根据区划过滤 按照指定行政区划进行过滤，填入后则只返回该省/直辖市信息 需填入adcode，为了保证数据的正确，强烈建议填入此参数
	 */
	...
	public String toString() {
		...
		builder.append("AreaParam [keywords=");
	...
	public String buildParamUrl() throws UnsupportedEncodingException {
		...
		if (StringUtils.isNotEmpty(subdistrict)) {
			builder.append("&subdistrict=" + subdistrict);
		}
