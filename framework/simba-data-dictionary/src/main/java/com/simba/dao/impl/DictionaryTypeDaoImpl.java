...
public class DictionaryTypeDaoImpl implements DictionaryTypeDao {
	...
	@Override
	@CacheEvict(cacheNames = "dictionaryType", key = "#dictionaryType.getId()")
	...
	@Override
	@CacheEvict(cacheNames = "dictionaryType", key = "#id")
	...
	@Override
	@Cacheable(cacheNames = "dictionaryType", key = "#id")
	...
	private String buildCondition(String sql, SearchDictionaryTypeForm searchDictionaryTypeForm, StatementParameter param) {
		sql += " where 1 = 1";
		if (searchDictionaryTypeForm.getCode() != null && StringUtils.isNotEmpty(searchDictionaryTypeForm.getCode() + "")) {
			sql += " and code  like '%" + searchDictionaryTypeForm.getCode() + "%'";
		}
		return sql;
	}
}
