...
public class MsgTemplateServiceImpl implements MsgTemplateService {
	...
	@Override
	public void updateTemplate(MsgTemplate template) throws NumberFormatException, APIConnectionException, APIRequestException {
	...
	public void addJpush(JpushTemplateForm templateJpush) throws APIConnectionException, APIRequestException {
		...
		if (list.size() == 0) {
			...
			template.setAliTemplateId(StringUtils.EMPTY);
	...
	public void addAli(AliTemplateForm templateAli) {
		...
		if (list.size() == 0) {
			...
			template.setJpushTemplateId(StringUtils.EMPTY);
	...
	public void updateJpushTemplatesStatus(List<MsgTemplate> templates) throws APIConnectionException, APIRequestException {
		for (MsgTemplate template : templates) {
			updateJpushTemplateStatus(template);
		}
	}

	private void updateJpushTemplateStatus(MsgTemplate template) throws APIConnectionException, APIRequestException {
		...
		if (status == AuditStatus.AUDITING.getId()) {
			...
			if (newStatus.getId() != AuditStatus.AUDITING.getId()) {
				logger.info(template.getName() + "短信模板" + AuditStatus.AUDITING.getDescription());
