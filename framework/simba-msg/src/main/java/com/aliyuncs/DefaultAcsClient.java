...
public class DefaultAcsClient implements IAcsClient {
	...
	public <T extends AcsResponse> HttpResponse doAction(AcsRequest<T> request, String regionId, Credential credential) throws ClientException, ServerException {
		...
		if (null != this.clientProfile) {
			...
			try {
				endpoints = clientProfile.getEndpoints(request.getProduct(), request.getRegionId(), request.getLocationProduct(), request.getEndpointType());
			} catch (Throwable e) {
	...
	@Deprecated
	@Override
	public <T extends AcsResponse> HttpResponse doAction(AcsRequest<T> request, boolean autoRetry, int maxRetryNumber, String regionId, Credential credential, ISigner signer, FormatType format,
	...
	private <T extends AcsResponse> HttpResponse doAction(AcsRequest<T> request, boolean autoRetry, int maxRetryNumber, String regionId, AlibabaCloudCredentials credentials, ISigner signer,
			FormatType format, List<Endpoint> endpoints) throws ClientException, ServerException {
