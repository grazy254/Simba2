...
public class TradeByWechatPay implements PayInterface {
	...
	public void dealResult(PayResult payResult) {
		logger.info(String.format("wechat recharge callbacke trade payResult: %s", payResult.toString()));
		TradeStatus status = TradeStatus.FAILED;
		if (payResult.getTotal_fee() > 0) {
			status = TradeStatus.SUCCESS;
		}
		TradeUser tradeUser = tradeUserService.getByOrderNO(payResult.getOut_trade_no(), TradeType.RECHARGE);
		logger.info("wechat recharge callback trade user account: " + tradeUser.getUserID());

		JsonResult rs = rechargeContext.finishTrade(tradeUser.getUserID(), payResult.getOut_trade_no(), payResult.getTransaction_id(), payResult.getOpenid(),
				CommonUtil.getGmtDate(payResult.getTime_end()), payResult.getErr_code_des(), payResult.getErr_code(), payResult.getTotal_fee(), status);
	...
	public void refund(RefundReq refundReq) {
		...
		TradeUser tradeUser = tradeUserService.getByOrderNO(refundReq.getOut_trade_no(), TradeType.RECHARGE);
		logger.info("wechat refund start trade user account: " + tradeUser.getUserID());

		JsonResult rs = refundContext.startTrade(tradeUser.getUserID(), "", refundReq.getOut_trade_no(), refundReq.getRefund_fee());
	...
	public void dealRefundCallback(RefundResult refundResult, RefundCallbackInfo callbackInfo) {
		logger.info(String.format("wechat refund callback trade RefundResult %s, RefundCallbackInfo %s: ", refundResult.toString(), callbackInfo.toString()));
		TradeUser tradeUser = tradeUserService.getByOrderNO(callbackInfo.getOut_trade_no(), TradeType.REFUND);
		...
		JsonResult rs = refundContext.finishTrade(tradeUser.getUserID(), callbackInfo.getOut_trade_no(), callbackInfo.getTransaction_id(), "", new Date(),
				CommonUtil.getGmtDate(callbackInfo.getSuccess_time()), "", "", callbackInfo.getRefund_fee(), status);
