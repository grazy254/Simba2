...
public class AliPayBillDaoImpl implements AliPayBillDao {
	...
	public void add(AliPayBill aliPayBill) {
		String sql = "insert into " + table + "( appid, body, totalAmount, subject, outTradeNo, tradeNo, productCode, goodType, storeId, sellId, timeoutExpress, createTime) values(?,?,?,?,?,?,?,?,?,?,?,?)";
		jdbc.updateForBoolean(sql, aliPayBill.getAppid(),aliPayBill.getBody(),aliPayBill.getTotalAmount(),aliPayBill.getSubject(),aliPayBill.getOutTradeNo(),aliPayBill.getTradeNo(),aliPayBill.getProductCode(),aliPayBill.getGoodType(),aliPayBill.getStoreId(),aliPayBill.getSellId(),aliPayBill.getTimeoutExpress(),aliPayBill.getCreateTime());
	}

	@Override
	public void update(AliPayBill aliPayBill) {
		String sql = "update " + table + " set  appid = ? , body = ? , totalAmount = ? , subject = ? , outTradeNo = ? , tradeNo = ? , productCode = ? , goodType = ? , storeId = ? , sellId = ? , timeoutExpress = ? , createTime = ?  where id = ?  ";
		jdbc.updateForBoolean(sql,aliPayBill.getAppid(),aliPayBill.getBody(),aliPayBill.getTotalAmount(),aliPayBill.getSubject(),aliPayBill.getOutTradeNo(),aliPayBill.getTradeNo(),aliPayBill.getProductCode(),aliPayBill.getGoodType(),aliPayBill.getStoreId(),aliPayBill.getSellId(),aliPayBill.getTimeoutExpress(),aliPayBill.getCreateTime(), aliPayBill.getId());
	}
	...
	private String buildCondition(String sql, AliPayBillSearchForm aliPayBillSearchForm, StatementParameter param) {
		sql += " where 1 = 1";
        if (aliPayBillSearchForm.getProductCode() != null 
        		&& StringUtils.isNotEmpty(aliPayBillSearchForm.getProductCode()+"")) {
			sql += " and productCode  = ?";
			param.set(aliPayBillSearchForm.getProductCode());
		}
        if (aliPayBillSearchForm.getOurTradeNo() != null 
        		&& StringUtils.isNotEmpty(aliPayBillSearchForm.getOurTradeNo()+"")) {
			sql += " and ourTradeNo  = ?";
			param.set(aliPayBillSearchForm.getOurTradeNo());
		}
        if (aliPayBillSearchForm.getTradeNo() != null 
        		&& StringUtils.isNotEmpty(aliPayBillSearchForm.getTradeNo()+"")) {
			sql += " and tradeNo  = ?";
			param.set(aliPayBillSearchForm.getTradeNo());
		}
        if (aliPayBillSearchForm.getStartCreateTime() != null 
        		&& StringUtils.isNotEmpty(aliPayBillSearchForm.getStartCreateTime()+"")) {
			sql += " and createTime  >= ?";
			param.set(aliPayBillSearchForm.getStartCreateTime());
		}
        if (aliPayBillSearchForm.getEndCreateTime() != null 
        		&& StringUtils.isNotEmpty(aliPayBillSearchForm.getEndCreateTime()+"")) {
			sql += " and createTime  <= ?";
