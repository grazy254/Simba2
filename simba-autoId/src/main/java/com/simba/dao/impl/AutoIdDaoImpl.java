 ...
 */
@Repository
public class AutoIdDaoImpl implements AutoIdDao {

	@Autowired
	private Jdbc jdbc;

	private static final String table = "autoId";

	@Override
	public void add(AutoId autoId) {
		String sql = "insert into " + table + "(id, num) values(?,?)";	
		jdbc.updateForBoolean(sql, autoId.getId(), autoId.getNum());
	}

	@Override
	public void update(AutoId autoId) {
		String sql = "update " + table + " set  num = ?  where id = ?  ";
		jdbc.updateForBoolean(sql,autoId.getNum(), autoId.getId());
	}
	...
	public List<AutoId> page(Pager page) {
		...
		return jdbc.queryForPage(sql, AutoId.class, page);
	}

	@Override
	public List<AutoId> listAll(){
		String sql = "select * from " + table;
		return jdbc.queryForList(sql, AutoId.class);
	}
	...
	public List<AutoId> listBy(String field, Object value) {
		String sql = "select * from " + table + " where " + field + " = ? ";
		return jdbc.queryForList(sql, AutoId.class, value);
	}

	@Override
	public List<AutoId> listByAnd(String field1, Object value1, String field2, Object value2) {
		String sql = "select * from " + table + " where " + field1 + " = ? and " + field2 + " = ? ";
		return jdbc.queryForList(sql, AutoId.class, value1, value2);
	}
	...
	public List<AutoId> pageBy(String field, Object value, Pager page) {
		...
		param.set(value);
		return jdbc.queryForPage(sql, AutoId.class, page, param);
	}
