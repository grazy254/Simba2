var CurrentOrgUser = {
	...
	"toAdd" : function() {
		window.self.location.href = contextPath + "/currentOrgUser/toAdd?orgID=" + $("#parentID").val();
	},
	...
	"batchDelete" : function() {
		...
		$.ajax({
			...
			success : function(data) {
				if (data.code == 200) {
					CurrentOrgUser.initUserList(0, Page.size);
				} else {
					parent.showInfo(data.msg);
				}
			}
	...
	"initUserList" : function(start, pageSize) {
		$.ajax({
			type : "get",
			url : contextPath + "/currentOrgUser/getUserList",
			data : {
				"orgID" : $("#parentID").val(),
				"pageStart" : start,
				"pageSize" : pageSize
		...
		$.ajax({
			...
			success : function(data) {
				var total = data.data;
				var pageHtml = Page.init(total, start, pageSize, "CurrentOrgUser.clickPager");
				$("#page").html(pageHtml);
			}
		});
	},
	...
	"clickPager" : function(start, pageSize) {
		CurrentOrgUser.initUserList(start, pageSize);
	},

	"toUpdate" : function(account) {
		window.self.location.href = contextPath + "/currentOrgUser/toUpdate?account=" + account;
	},
	...
	"toList" : function() {
		var orgID = $("#orgID").val();
		window.self.location.href = contextPath + "/currentOrgUser/list?orgID=" + orgID.split(",")[0];
	},
