...
import com.simba.dao.TradeChannelDao;
...
public class TradeChannelDaoImpl implements TradeChannelDao {
	...
	public void add(TradeChannel tradeChannel) {
		String sql = "insert into " + table + "( tradeAccountID, channelName, channelType, orderCreateTime, paymentTime, orderNO, openID, errorMsg, errorCode, createTime) values(?,?,?,?,?,?,?,?,?,?)";
		jdbc.updateForBoolean(sql, tradeChannel.getTradeAccountID(),tradeChannel.getChannelName(),tradeChannel.getChannelType(),tradeChannel.getOrderCreateTime(),tradeChannel.getPaymentTime(),tradeChannel.getOrderNO(),tradeChannel.getOpenID(),tradeChannel.getErrorMsg(),tradeChannel.getErrorCode(),tradeChannel.getCreateTime());
	}

	@Override
	public void update(TradeChannel tradeChannel) {
		String sql = "update " + table + " set  tradeAccountID = ? , channelName = ? , channelType = ? , orderCreateTime = ? , paymentTime = ? , orderNO = ? , openID = ? , errorMsg = ? , errorCode = ? , createTime = ?  where id = ?  ";
		jdbc.updateForBoolean(sql,tradeChannel.getTradeAccountID(),tradeChannel.getChannelName(),tradeChannel.getChannelType(),tradeChannel.getOrderCreateTime(),tradeChannel.getPaymentTime(),tradeChannel.getOrderNO(),tradeChannel.getOpenID(),tradeChannel.getErrorMsg(),tradeChannel.getErrorCode(),tradeChannel.getCreateTime(), tradeChannel.getId());
	}
